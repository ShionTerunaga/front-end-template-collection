...
const { style, clear, figures, wrap, entriesToDisplay } = __nccwpck_require__(1089);
...
const { erase, cursor } = __nccwpck_require__(4975);
const { DatePart, Meridiem, Day, Hours, Milliseconds, Minutes, Month, Seconds, Year } = __nccwpck_require__(7569);
...
const { cursor } = __nccwpck_require__(4975);
const Prompt = __nccwpck_require__(1244);
const { clear, figures, style, wrap, entriesToDisplay } = __nccwpck_require__(1089);
...
async function libCli(optionLibs) {
    const { optionConversion, createSome, createNone } = option_1.optionUtility;
    const { createOk, checkPromiseReturn, createNg } = result_1.resultUtility;
    const { onPromptState } = command_core_1.commanderCore;
    if (optionLibs.isSome && (0, is_1.isBoolean)(optionLibs.value) && optionLibs.value) {
...
const validate_npm_package_name_1 = __importDefault(__nccwpck_require__(3273));
...
async function addPackage({ root, css, libs }) {
    const { createOk, checkPromiseReturn, checkPromiseVoid } = result_1.resultUtility;
    if (libs.length === 0) {
...
async function reactInstaller({ appPath, material }) {
    ...
    if (!(0, react_is_1.isReactCss)(styleSheet.value)) {
...
async function reactCli() {
    const { optionReactFramework, optionCss, optionUseAllComponents } = command_core_1.commanderCore;
    ...
    const resultSelected = await (0, react_lib_1.libCli)(optionUseAllComponents);
...
const { CommanderError, InvalidArgumentError } = __nccwpck_require__(6483);
...
const { Argument, humanReadableArgName } = __nccwpck_require__(3230);
