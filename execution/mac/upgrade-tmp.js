...
async function main(TARGET_DIR_IN_ZIP) {
    const tmpBase = (0, node_path_1.join)((0, node_os_1.tmpdir)(), `upgrade-tmp-${Date.now()}`);
    const zipPath = (0, node_path_1.join)(tmpBase, `${REPO}.zip`);
    const extractDir = (0, node_path_1.join)(tmpBase, "extract");
    const toolsDir = (0, node_path_1.join)((0, node_os_1.homedir)(), DIR);
    (0, node_fs_1.mkdirSync)(tmpBase, { recursive: true });
    (0, node_fs_1.mkdirSync)(extractDir, { recursive: true });
    console.log("📦 Downloading ZIP...");
    const zipUrl = `https://github.com/${OWNER}/${REPO}/archive/refs/heads/${BRANCH}.zip`;
    (0, node_child_process_1.execSync)(`curl -L "${zipUrl}" -o "${zipPath}"`, { stdio: "inherit" });
    console.log("🧩 Extracting...");
    const zip = new adm_zip_1.default(zipPath);
    zip.extractAllTo(extractDir, true);
    // 展開後フォルダ（例: create-react-template-release）
    const [rootFolder] = (0, node_fs_1.readdirSync)(extractDir);
    const extractedRoot = (0, node_path_1.join)(extractDir, rootFolder);
    const sourceDir = (0, node_path_1.join)(extractedRoot, TARGET_DIR_IN_ZIP);
    if (!(0, node_fs_1.existsSync)(sourceDir)) {
        throw new Error(`❌ target directory not found in zip: ${sourceDir}`);
    }
    for (const item of (0, node_fs_1.readdirSync)(toolsDir)) {
        if (item === (0, node_path_1.basename)(__filename))
            continue; // 自分自身は消さない
        if (item === ".git")
            continue;
        const p = (0, node_path_1.join)(toolsDir, item);
        (0, node_fs_1.rmSync)(p, { recursive: true, force: true });
        console.log("removed:", p);
    }
    console.log("📁 Copying files from:", sourceDir);
    copyRecursive(sourceDir, toolsDir);
    console.log("🧹 Cleaning temp files...");
    (0, node_fs_1.rmSync)(tmpBase, { recursive: true, force: true });
    // Try to make the new tools directory available as a global CLI via npm link
    try {
        console.log("🔗 Running `npm install -g .` in", toolsDir);
        (0, node_child_process_1.execSync)("npm install -g .", { stdio: "inherit", cwd: toolsDir });
        console.log("✅ npm install completed");
    }
    catch (e) {
        console.error("⚠️ npm install failed:", e instanceof Error ? e.message : e);
        console.error("You can run `npm install -g .` manually in", toolsDir);
    }
    console.log("✅ Upgrade complete!");
}
function copyRecursive(src, dest) {
    for (const item of (0, node_fs_1.readdirSync)(src)) {
        const s = (0, node_path_1.join)(src, item);
        const d = (0, node_path_1.join)(dest, item);
        const st = (0, node_fs_1.statSync)(s);
        if (st.isDirectory()) {
            (0, node_fs_1.mkdirSync)(d, { recursive: true });
            copyRecursive(s, d);
        }
        else {
            (0, node_fs_1.renameSync)(s, d);
